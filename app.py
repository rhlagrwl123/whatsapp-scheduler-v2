...
def parse_with_gemini(prompt, user_date, user_time):
    api_key = os.getenv("GEMINI_API_KEY", "")
    if not api_key:
        return {"valid": False, "error": "Gemini API key not set"}
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={api_key}"
    body = {
        "contents": [{
            "parts": [{
                "text": f"""Today is {user_date}. Current time: {user_time}.
Parse this reminder request and return ONLY valid JSON, no markdown, no explanation:
"{prompt}"

Return exactly this JSON:
{{"title":"short task name","message":"friendly WhatsApp reminder message with emoji","datetime":"YYYY-MM-DD HH:MM","date_display":"e.g. Tomorrow, 5th March","time_display":"e.g. 7:00 PM","valid":true,"error":null}}

If you cannot parse it, return: {{"valid":false,"error":"reason"}}"""
            }]
        }]
    }
...
async function ping(){
  try{
    ...
    if(r.ok){document.getElementById("dot").className="dot on";document.getElementById("stxt").textContent="server live"}
    else throw 0;
  }catch{document.getElementById("dot").className="dot";document.getElementById("stxt").textContent="offline"}
}
...
async function go(){
  ...
  try{
    const now=new Date();
    const r1=await fetch("/parse",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({prompt,date:now.toLocaleDateString("en-IN",{weekday:"long",year:"numeric",month:"long",day:"numeric"}),time:now.toLocaleTimeString("en-IN")})});
...
def parse_reminder():
    ...
    if not data or "prompt" not in data:
        return jsonify({"valid": False, "error": "No prompt provided"}), 400
    try:
        result = parse_with_gemini(data["prompt"], data.get("date", ""), data.get("time", ""))
