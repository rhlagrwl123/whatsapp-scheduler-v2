...
async function ping(){
  try{
    const r=await fetch(window.location.origin+"/status",{signal:AbortSignal.timeout(5000)});
...
async function go(){
  ...
  try{
    ...
    const r2=await fetch(window.location.origin+"/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:p.message,datetime:p.datetime})});
...
async function del(id){
  try{await fetch(window.location.origin+"/delete/"+id,{method:"DELETE"})}catch{}
